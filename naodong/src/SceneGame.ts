class SceneGame extends eui.Component implements eui.UIComponent {
	...
	private getSelectWords(index:number)
	{
		...
		{
			let mystr = "";
			mystr = LevelDataManager.getInstance().GetLevelData(index).chaotic;
			return mystr;
		}
		else
		{
			let oldstr = "";
			for (let i = 0; i < 20; i++) {
				oldstr += LevelDataManager.getInstance().GetLevelData(index + i + 1).chaotic;//取到后面20关的答案
			}
			let myreult = "";
			//本关的答案
			myreult += LevelDataManager.getInstance().GetLevelData(index).result;
			//筛选8个字出来
			let newstr = "";
	...
	private xiaoguo() {
		egret.Tween.get(this.caiziBtn).to({ scaleX: 1.2, scaleY: 1.2 }, 1000).to({ scaleX: 1, scaleY: 1 }, 1000).call(() => {
